...
public class MainForm extends javax.swing.JFrame {
    ...
    public void connect(){
        ...
        try {
            ...
            dos = new DataOutputStream(socket.getOutputStream()); //DataOutputStream là một lớp trong Java, cung cấp các phương thức để ghi các dữ liệu 
            ...
            new Thread(new ClientThread(socket, this)).start(); // nhận từ tin nhắn từ máy chủ
            jButton1.setEnabled(true);//cho phép nút gửi dc bấm
            // đã được kết nối
            isConnected = true;
    ...
    */
    public void showOnLineList(Vector list){ //vector :gồm các dsach người dùng
        try {
            txtpane2.setEditable(true);
            txtpane2.setContentType("text/html");//đặt kiểu tin nhắn thành html
            StringBuilder sb = new StringBuilder();
            Iterator it = list.iterator();
            sb.append("<html><table>"); //nối chuỗi tên ng dùng và icon online
            while(it.hasNext()){ // có người dùng mới đăng nhập vào
                Object e = it.next();
            ...
            txtpane2.setEditable(false);// vô hiệu hoá chỉnh sửa
        } catch (Exception e) {
    ...
    */
    private void sampleOnlineList(Vector list){
        ...
        while(i.hasNext()){ // nếu có ng dùng mới online
            Object e = i.next(); //ten ng dùng
            /*  Hiển thị Username Online   */
            JPanel panel = new JPanel(); // tạo panel cho mỗi ng dùng
            panel.setLayout(new FlowLayout(FlowLayout.LEFT));
            panel.setBackground(Color.white);
            
            Icon icon = new ImageIcon(this.getClass().getResource("/images/online.png"));
            JLabel label = new JLabel(icon);
            label.setText(" "+ e);// icon online + tên ng dùng 
            panel.add(label);// thêm icon+tenngdung  vào panel
            int len = txtpane2.getDocument().getLength();
            txtpane2.setCaretPosition(len);
            txtpane2.insertComponent(panel);// bảng txtpane2 sẽ thêm panel của mỗi người dùng
            /*  Append Next Line   */
            sampleAppend();// xuống dòng nối thêm dùng mới vào txtpane2
        }
    ...
    */
    ...
    private void initComponents() {
        ...
        sendFileMenu.setLabel("Gửi File");
        ...
        sendFileMenu.getAccessibleContext().setAccessibleName("gửi file");
    ...
    private void sendFileMenuActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_sendFileMenuActionPerformed
        ...
        if(!attachmentOpen){ //kiểm tra xem liệu đã có một hộp thoại chia sẻ tập tin khác đang mở hay chưa.
    ...
    private void LogoutMenuActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_LogoutMenuActionPerformed
        ...
        if(confirm == 0){
            try {
                ...
                setVisible(false);
                /** Login Form **/
                new LoginForm().setVisible(true); // form bị tắt đi
            } catch (IOException e) {
                System.out.println(e.getMessage());
            }
        }
    ...
    private void jTextField1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jTextField1ActionPerformed
        // TODO add your handling code here:
        try {
            String content = username+" "+ evt.getActionCommand();//evt.getActionCommand() : trả về tin nhắn khi bấm phím enter gửi tin nhắn
            dos.writeUTF("CMD_CHATALL "+ content);// gửi thông báo đến máy chủ bằng CMD_ChatALL
            appendMyMessage(" "+evt.getActionCommand(), username);// trả về tin nhắn vào khung tin nhắn với tương ứng với user name
            jTextField1.setText("");//set textbox rỗng
        } catch (IOException e) {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        try {
            ...
            appendMyMessage(" "+jTextField1.getText(), username);// ghi tin nhắn vào cửa sổ chat
            jTextField1.setText("");
        } catch (IOException e) {
            appendMessage(" Không thể gửi tin nhắn đi bây giờ, không thể kết nối đến Máy Chủ tại thời điểm này, xin vui lòng thử lại sau hoặc khởi động lại ứng dụng này.!", "Lỗi", Color.RED, Color.RED);
        }
